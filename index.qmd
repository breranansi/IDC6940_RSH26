...
#### First Sparse Run
It would seem remiss to not *at least* look at one of the deterministic models.  The first one to *work* without requiring long depreciated versions of Python is @muthyala2025symantic so we set up the run to be equivalent the the *Default Run* above.  Our "winning" equation is:

$$ M_w=-a_{00} \gamma_o +\frac{a_{01} \gamma_o}{T_b}+a_{03} T_b -a_{04}$$ {#eq-res-mth-1}

This is shockingly simple compared to the equations we have generated so far.  The quality of the fit, however, is disapointing:
...
$$ M_w=-a_{00} \gamma_o T_b +a_{01}T^2_b +a_{02} e^{\gamma_o} +a_{03}$$ {#eq-res-mth-2}
...
A different approach is warranted with the deterministic models.  It is likely that these are *underfit*, meaning that we can expand the number of unique terms the algorithm initially considers, or the number of terms we want to keep.
...
The deterministic symbolic regression libaries don't need random number seeds.  The equations are definitely more *explainable*, but appear to be less accurate.  Does this accuracy matter?  Is it real?  The answer to these questions depend on how good we think our measurements are!
...
2.  Evolutionary Symbolic Regression will typically generate novel equations from the
...
4.  The ability of Evolutionary Symbolic Regression to easily find alternative
...
6.  Piping the "winning" equation through a symbolic mathematics package
    to simplify and consolidate it is a good idea.  The effect on the deterministic equaions were very slight.
