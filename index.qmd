...
The `PySR` library is silent over control of constants. In its implementation, the equation forms are generated first, which for our toy example would be:
...
@virgolin2022coefficient present two algorithms that adjust the value of the constants as an integral part of the evolutionary process, but conclud that "coefficient mutation does not always make a difference". This is disappointing.
...
hist(df$SG,
     ...
     col="lightgreen")
```
This distribution is skewed to the right, but not as strongly as molecular mass.  No signs of a bi modal distribution here.  Let take a peek at the box plot:
...
Notice that the largest variation in molecular mass corresponds to the specific gravity range 0.8-0.9, which, acording to @fig-his_sgo is our mode.
