...
Deterministic methods such as brute-force search [@udrescu2020ai], mathematical programming [@austel2017globally] and sparse regresion [@muthyala2025symantic] have been employed from the early times until today.  Typically, they don't solve the speed problem, but are thought to produce more interpretable equations.
...
There are some challenges. The constant $a_{02}$ is only slightly larger than the largest $\gamma$ (specific gravity) in the dataset.  This limits the extrapolation power of this equation to higher specific gravities than seen in this dataset.  Extrapolation is one of the strengths of Symbolic Regression [@sahoo2018learning]
...
$M_w=a_{00} \cdot a_{01}^{SG^{-a_{02}} + a_{03} TBP} + a_{04}$
...
$\displaystyle M_w= - TBP \left(a_{00} TBP - a_{01}\right) \left(a_{02} \cdot 10^{-6} \left(a_{03} SG - 2 TBP\right) \left(TBP - a_{04}\right) - 1\right) + a_{05}$

Accordingly, we re-set the random number generator and tried again:

$\displaystyle M_w= a_{00} TBP + a_{01} e^{- SG^{2} + a_{02} SG + a_{03} TBP}$

This is a dramatically different equation.  This experience suggests that the *real* reason the deterministic algorithms are still under active development today is to avoid this kind of ambiguity. We have shown (with the Toy dataset) that equation bloat can often be reduced automatically with symbolic mathematics packages.
...
Also noteable in this run is the absence of the logarithm function to linearize the relationship between these two variables:

![Linearizing](around3_files/figure-html/cell-14-output-1.png)

This "obvious" human observation was clearly not an optimum transformation, or was eliminated by the evolutionary algorithm before it could reach its full potential.
