...
The reader may remember that our molecular mass was *not* normally distributed.  Accordingly, here we transform our dependent variable to see if this improves the distribution of our residuals.  Our optimum exponent works out to be $-0.3624$ according to the python `stats` library, which is fairly close to zero.
...
This run produced the same equation form at 100, 500 and 1000 iterations, which was unusual for the `PySR` runs so far:

$$ M_w^t = \left (\gamma_o \left(a_{00} T_b -a_{01} \right) log{(\gamma_o)} + a_{02} \right) log{(T_b)} $$ {#eq-res-boxcox}

But we need to remember to the reverse Box-Cox transform:

$$ M_w = {\left(\lambda M_w^t +1 \right)}^{\frac{1}{\lambda}} $$ {#eq-unboxcox}

The correlation coefficient for the longest run (1000 iterations) is in line with the Aeon runs discussed above:
...
Residuals are small for molecular masses below about 350, but explode above 1000.  Statistically speaking, this may be good if we are more uncertain about the accuracy of the larger molecular masses, but from an engineering point of view this equation is a disaster for exactly the same reason.
...
#### Second Sparse Run
This leads to a second deterministic experiment, in which we add the logarithm and exponential functions, but leave the division operator in place. Our new equation is *not* complex either:
