...
#### Power Run
Our Second experiment is to drop the "division" binary operator $(\frac{a}{b})$ and add the "power" binary operator $(a^b)$ as the latter is a popular component of the existing correlations.
...
One can argue that nested exponents $(a^{b^c})$ aren't very explainable.  However the Stratiev correlation does this with Euler's number $\left ( a \cdot e^{b \cdot e^{c}} \right )$.  In the hydrocarbon flow literature, nested logarithms $ln(ln(a))$ are more common. 
...
$\displaystyle M_w= - T_b \left(a_{00} T_b - a_{01}\right) \left(a_{02} \cdot 10^{-6} \left(a_{03} \gamma_o - 2 T_b \right) \left(T_b - a_{04}\right) - 1\right) + a_{05}$

Accordingly, we re-set the random number generator and tried again:

$\displaystyle M_w= a_{00} T_b + a_{01} e^{- \gamma_o^{2} + a_{02} \gamma_o + a_{03} T_b}$
...
#### Aeon Run
This fourth experiments is the same as the third experiment, except that we run it for ten times as long.  Here, we are interested in whether different starting points will eventually converge to similar equations.  The answer appears to be *no*.  Here is the first equation:
...
The second equation is truly facinating for two reasons.  Neither the power operator $\left (a^b \right )$ nor the division operator $\left (\frac{a}{b} \right)$ were provided to the algorithm for this run, and yet we have found the middle expression $\frac{e^{-\gamma_o^2+{log(T_b)}^3}}{T_b^{a_{01}}}$.
...
Interestingly, switching between a cloud-based compute service (Google Colab) and a laptop with a notebook file that was suitably "locked down" produced the same equations. This was reassuring. Moving that same file to a desktop with a different brand of processor but otherwise identical set-up produced *different* equations. This was terrifying.
