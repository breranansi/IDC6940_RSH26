...
<p style="color:Tomato">Description of health stuff challenge goes here</p>
...
<p style="color:Tomato">A comment connecting health stuff to molecular mass goes here</p>
...
Box-Cox transform, where the aim is to make the population more normal.
...
And the evolutionary algorithm will typically present an output looking
like this:

$$x_0-(x_0+0.013196754)+1.0131962+ \frac{x_0 (-132.5)- -141.5}{x_0}$$ {#eq-toy-raw}

This looks terrible, but we can clean it up using a symbolic
mathematics library (or high-school algebra):
...
This was enough to throw-off the estimation of the constants a bit, but
the form of the equation was preserved:

$$-132.05688+\frac{141.88547}{x_0}$$ {#eq-toy-noise}

### Belly of the Beast

There are a large number of hyperparameters in a typical symbolic
regression model that we have defaulted in the Toy dataset model above.
...
Notice that the `"best"` generated toy equation is significantly longer
...
Notation", and a post-fix variant was used on scientific calculators in
the 1970's as it was more efficient. The learning curve, however, was
brutal for mere mortals, so that owning one of these 'RPN' artifacts
became a status symbol in the engineering community well into the
1990's. Yeah, I know this is irrelevant.

`maxdepth=None`

This is the maximum depth of a generated equation. Our toy equation has
a depth of three: ![Toy Tree](API_tree.png)
...
longer and contain more transcendental functions.
...
<!-- Description of health stuff dataset goes here -->
...
<p style="color:Tomato">next steps with health stuff goes here</p>
...
$$M_w = a_{00} + a_{01} K_w + a_{02} K_w^2 + a_{03} T_b K_w + a_04 T_b K_w^2 + a_{05} T_b^2 K_w + a_{06} T_b^2 K_w^2$$ {#eq-harlu}
...
$$X_1 = \left (1+ a_{04} γ_o + a_{05}γ_o^2 \right ) \left (a_{06} + \frac{a_ {07}}{T_b} \right ) \cdot 10^7$$ {#eq-kes-1}

$$X_2 = \left (1+ a_{08} γ_o+ a_{09} γ_o^2 \right ) \left (a_{10} + \frac{a_{11}}{T_b} \right ) \cdot 10^{12}$$ {#eq-kes-2}

##### American Petroleum Institute (1977)

$$M_w = a_{00} e^ {\left (a_{01} T_b \right )} e^{\left (a_{02} γ_o \right )} T_b^{a_{03}} γ_o^ {a_{04}}$$ {#eq-api-77}
...
$$M_w = a_{00} T_b^{a_{01}} γ_o^{a_{02}} e^{\left (a_{03} T_b + a_{04} γ_o + a_{05} T_b γ \right )}$$ {#eq-riazi-87}

##### Goossens (1996)

$$M_w = a_{00} T_b^{X_0}$$ {#eq-goossens}

$$X_0 =\frac {a_{03} + a_{04} ln {\left (\frac{T_b}  {a_{05} - T_b} \right )}}  {a_{01} γ_o + a_{02}}$$ {#eq-goo-0}

##### Linan (2011)

$$M_w = a_{00} e^{\left (a_{01} T_b \right )} e^{\left (a_{02} γ_o \right )} T_b^ {a_{03}} γ_o^{a_{04}}$$ {#eq-linan}
...
$$M_w = a_{00} + a_{01} e^{\left [a_{02} e^{\left (a_{03} \frac{T_b^{a_{06}}}{γ_o^{a_{05}}} \right )} \right ]}$$ {#eq-stratiev}
...
<p style="color:Tomato">Exploration of health stuff dataset goes here</p>
...
$$M_w=a_{00}+\frac{a_{01}}{\gamma_o-a_{02}}+\frac{T_b(a_{03}T_b-a_{04})}{\gamma_o(a_{05}-\frac{a_{06}}{T_b})}$$ {#eq-res-def}
...
molecular mass data than the equation presented *with* this data in the
Goosens paper:
...
$$M_w=a_{00} \cdot a_{01}^{\gamma_o^{-a_{02}} + a_{03} T_b} + a_{04}$$ {#eq-res-pow}
...
$$ M_w= - T_b \left(a_{00} T_b - a_{01}\right) \left(a_{02} \cdot 10^{-6} \left(a_{03} \gamma_o - 2 T_b \right) \left(T_b - a_{04}\right) - 1\right) + a_{05}$$ {#eq-res-exp-1}

Accordingly, we re-set the random number generator and tried again:

$$ M_w= a_{00} T_b + a_{01} e^{- \gamma_o^{2} + a_{02} \gamma_o + a_{03} T_b}$$ {#eq-res-exp-2}
...
$$ M_w=(a_{00}T_b-a_{01})e^{a_{02}\cdot 10^{-9}T_b^2(a_{03}\cdot \gamma_0 +a_{04}\cdot T_b)} $$ {#eq-res-eon-1}

And the second equation:

$$ M_w=-a_{00}\cdot \gamma_o+\frac{e^{-\gamma_o^2+{log(T_b)}^3}}{T_b^{a_{01}}} +a_{02}\cdot T_b$$ {#eq-res-eon-2}
...
+----------+----------+----------+----------+----------+----------+
|          | R        | Defaul   | Goossen  | Powe     | Ex       |
|          | aw<br>Mw | t<br>Run | s<br>Cor | r<br>Run | ponentia |
|          |          |          | relation |          | l<br>Run |
|          |          |          |          |          | #2       |
+==========+=========:+=========:+=========:+=========:+=========:+
| Raw Mw   | 1.000000 | 0.999957 | 0.999939 | 0.996964 | 0.998954 |
+----------+----------+----------+----------+----------+----------+
...
victory over overtraining, as the Hosseinifar dataset is far better
behaved than the Goossens dataset.

A better approach is probably to start increasing `parsimony` or
decreasing `maxsize` or `maxdepth` until the correlation coefficients
start to deteriorate.

<p style="color:Tomato">health stuff dataset runs go here</p>

#### General Observations

The "UOP Characterization Factor"
$\left (K_w =\frac{\sqrt[3]T_b}{\gamma_o} \right)$ which (at least) two
...
Interestingly, switching between the cloud-based compute service [Google
Colab](https://colab.research.google.com/) and an ancient laptop with a
Jupyter notebook file that was suitably "locked down" produces the same
equations. This is reassuring. Moving that same file to a modern desktop
with a different processor but otherwise identical set-up produces
*different* equations. This is terrifying.
...
may know exactly what the governing equations are, producing lookup
...
concept to the idea of pre-training a tranformer network to do symbolic
regression.

This paper has shown that high fidelity models can be constructed from
"cheap" components (= arithmetic operators). Maybe *explainability* is
overrated, and functions carefully built using symbolic regression may
be competitive with old-man Gauss!
...
In summary, we have explored Symbolic Regression using evolutionary
algorithms and two very different datasets. We have not spent much time
on different kinds of engines (e.g. neural networks, mathematical
programming) or tested the extrapolation chops of this technology.
...
2.  Symbolic Regression will typically generate novel equations from the
    human point of view, but these equations can be as accurate than the
...
3.  The "explainability" of the generated equations are probably
    ...
    space searched by the Symbolic Regression algorithm.

4.  The ability of Symbolic Regression to easily find alternative
    equations of about the same accuracy means that humans can repeat
    the workflow until an equation that is more "explainable" than the
    others is generated.
...
6.  Piping the "winning" equation through a symbolic mathematics package
    to simplify and consolidate it is a good idea.
...
<p style="color:Tomato">Health stuff conclusions go here</p>
