...
Video: [here](https://drive.google.com/file/d/1R-Rdh6zhaPpIMUNRURnB_zTObqIdnXzf/view?usp=drive_link){target="_blank"}
...
[Box-Cox transform](https://en.wikipedia.org/wiki/Power_transform), (See also @box1964analysis)
...
[@muthyala2025symantic] have been employed.

Methods which rely on [information theory](https://en.wikipedia.org/wiki/Information_theory) have also been used,
especially of late.  (See also @shannon2001mathematical) The idea here is to replace the random changes to
the equations that are the heart of the evolutionary approach with
changes based on what we know about the data. Sometimes this information
...
When sparse regression is used in symbolic regression, a large number of potential terms are generated using combinations of the independent variables.  Technically, the resulting equation is only *sparse* if the number of terms exceeds the number of rows in the dataset, but usually this is not a problem.  Then regularization is applied to force *most* of the generated terms to zero, dramatically shortening the length of the equation.

As later in this paper we will be comparing an algorithm based on sparse regression against one based on evolution, we provide this quick recount of an informal experiment we made on our toy dataset.

We tried to fit the following equation to the toy dataset using vanilla linear regression:

$$ \hat{y} \approx \beta_{00}+\beta_{01}x_0+\beta_{02}x_0^2+\beta_{03}x_0^3$$ {#eq-spa-cub}

and got a very good fit. We then added an inverse term:

$$ \hat{y} \approx \beta_{00}+\beta_{01}x_0+\beta_{02}x_0^2+\beta_{03}x_0^3+\frac{\beta_{04}}{x_0}$$ {#eq-spa-inv}

and without even using regularization, all the terms except $\beta_{00},\beta_{04}$ were reduced to zero and we obtained a perfect fit.

This encouraged us to add exponential and logarithmic terms:

$$ \hat{y} \approx \beta_{00}+\beta_{01}x_0+\beta_{02}x_0^2+\beta_{03}x_0^3+\frac{\beta_{04}}{x_0}+\beta_{05}log(x_0)+\beta_{06}e^{x_0}$$ {#eq-spa-exp}

and again we were thrilled to see all terms except $\beta_{00},\beta_{04}$ were reduced to (near) zero .

However, when we removed the inverse term:

$$ \hat{y} \approx \beta_{00}+\beta_{01}x_0+\beta_{02}x_0^2+\beta_{03}x_0^3+\beta_{05}log(x_0)+\beta_{06}e^{x_0}$$ {#eq-spa-no-inv}

All the remaining terms got very large, even though the fit continued to be excellent.

This suggested to us that the regularization *was* an important part of sparse regression when it is used for symbolic regression.

### AI Statement

No generative AI products were used in the writing of this paper[^2].  Accordingly, the reader will encounter a shameless mixture of American and British spelling and idioms.

However, AI *was* used to coerce recalcitrant libraries to stop throwing error codes and actually produce something useful.  Fortunately for the reader, this was only partially successful, or this paper would be a lot longer.
...
Visually, the residuals *appear* to be normally distributed, but running the Shapiro-Wilk test (@shapiro1965analysis) reveals a probability of only 0.001 of the residuals being normally distributed.  So much for eye-balling!
...
Maybe it is appropriate at this juncture to include this:

::: {.callout-tip title="Formal Statistical Test"}

$H_0$:  The residuals created by using @eq-res-def to predict the molecular masses in the Goossens dataset are normally distributed.

$H_1$: The residuals are not normally distributed.

__Level of Significance__: 0.05

__Shapiro-Wilk Statistic__: 0.932714

__Shapiro-Wilk p-Value__: 0.001

__Conclusion__: We will reject the null hypothesis as the calculated p-value is less than the level of significance.
:::
...
[^2]: No algorithms were harmed during the making of this paper.
[^3]: A quick note for humans on this second equation: ${log(T_b)}^3 = {(ln(T_b))}^3 \neq ln(T^3_b)$  This was made clear by inspecting the Polish Notation version of the equation.  Just sayin'.
